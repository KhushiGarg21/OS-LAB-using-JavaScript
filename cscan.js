...
function execute() {
  ...
  if (headPosition.value === "" && allOk) {
    str =
      "The starting head position needs to be mentioned! It cannot be left blank.";
    document.getElementById("alert-wrapper").innerHTML = `
        <div class="alert alert-danger alert-dismissible fade show" role="alert" style="margin: 15px;">
            <strong>Warning!</strong> ${str}
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  ...
  if (
    (Number(headPosition.value) < 0 ||
      Number(headPosition.value) > Number(maxTrack.value)) &&
    allOk
  ) {
    str =
      "The starting head position of must lie between 0 and maximum track number.";
    document.getElementById("alert-wrapper").innerHTML = `
        <div class="alert alert-danger alert-dismissible fade show" role="alert" style="margin: 15px;">
            <strong>Warning!</strong> ${str}
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  ...
  trackRequests.forEach((x) => {
    if (x < 0 || (x > Number(maxTrack.value) && allOk)) {
      str =
        "All the track requests must lie between 0 and maximum track number.";
      document.getElementById("alert-wrapper").innerHTML = `
            <div class="alert alert-danger alert-dismissible fade show" role="alert" style="margin: 15px;">
                <strong>Warning!</strong> ${str}
                        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  ...
  if (trackRequests.length != Number(requests.value)) {
    str =
      "Please make sure that the number of track requests in the array match the total number of requests.";
    document.getElementById("alert-wrapper").innerHTML = `
        <div class="alert alert-danger alert-dismissible fade show" role="alert" style="margin: 15px;">
            <strong>Warning!</strong> ${str}
                    <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
  ...
  if (allOk) {
    ...
    trackRequests.push(0);
    trackRequests.push(xrange);
    ...
    progressWrapper.innerHTML = `<div id="progressBarContainer" class="progress animate__animated animate__backInUp">
    ...
    function done() {
      ...
      genPDF.addEventListener("click", () => {
        ...
        var seekCalc =
          ...
          seekOperationsCalculations(trackRequests, head);
        var calc = doc.splitTextToSize(seekCalc, 180);
        doc.text(calc, 20, 107);
        doc.setFont("Times", "bold");
        doc.text(
          "Thus, total seek time = " +
            String(seekOperations(trackRequests, head)) +
            "ms (Considering successive track seek time as 1ms)",
          20,
          121
        );
        var seekTime =
          "Average Seek Time = " +
          String(Math.round((xrange / 3) * 100) / 100) +
          "ms (Time taken by the header to move across one third of total tracks)";
        seekTime = doc.splitTextToSize(seekTime, 180);
        doc.text(seekTime, 20, 131);

        var note =
          'If your disk takes "x" amount of time (in milliseconds) to seek across successive tracks, then multiply the above results by "x" to get the correct results';
    ...
    function displaySeekOp() {
      ...
      seekOp1.id = "temp";
      seekOp1.style.fontWeight = "700";
      seekOp1.style.margin = "5px";
      str = "Total Seek Time: " + seekOperations(trackRequests, head) + " ms";
      seekOp1.append(document.createTextNode(str));
      seekOp1.classList.add("animate__animated");
      seekOp1.classList.add("animate__backInUp");
      document.getElementById("seek").append(seekOp1);

      var seekOp2 = document.createElement("h4");
      seekOp2.id = "temp1";
      seekOp2.style.fontWeight = "700";
      seekOp2.style.margin = "5px";
      str =
        "Average Seek Time: " + Math.round((xrange / 3) * 100) / 100 + " ms";
      seekOp2.append(document.createTextNode(str));
      seekOp2.classList.add("animate__animated");
      seekOp2.classList.add("animate__backInUp");
      document.getElementById("seek").append(seekOp2);

      var dIcon = document.createElement("a");
      dIcon.id = "url";
      dIcon.download = "CSCAN.jpeg";
      var dButton = document.createElement("button");
      dButton.type = "button";
      dButton.className =
        "dButton btn btn-outline-dark animate__animated animate__backInUp";
      ...
      pdfButton.className =
        "pdfButton btn btn-outline-dark animate__animated animate__backInUp";
      pdfButton.style.padding = "0";
      pdfButton.id = "genPDF";
      pdfButton.innerHTML = `
